...
class InquiryRepublikaNewsBola extends StatefulWidget {
  const InquiryRepublikaNewsBola({Key? key}) : super(key: key);

  @override
  State<InquiryRepublikaNewsBola> createState() => _InquiryRepublikaNewsBolaState();
}

class _InquiryRepublikaNewsBolaState extends State<InquiryRepublikaNewsBola> {
  final newsRepo = Get.put(RepublikaNewsRepository());
  final String author = "Republika Online - Sepakbola Feed";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/republika/bola';
  ...
  void processData(BeritaModel data) async {
    ...
    await RepublikaNewsRepository.instance.getAllNewsRepublikaBola(tempCtr-2);
    await RepublikaNewsRepository.instance.getAllNewsRepublikaBola(tempCtr-3);
    List<String> listJudul = RepublikaNewsRepository.instance.getListJudulNewsRepublikaBola();
