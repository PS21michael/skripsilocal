...
class InquiryTempoNewsBisnis extends StatefulWidget {
  const InquiryTempoNewsBisnis({Key? key}) : super(key: key);

  @override
  State<InquiryTempoNewsBisnis> createState() => _InquiryTempoNewsBisnisState();
}

class _InquiryTempoNewsBisnisState extends State<InquiryTempoNewsBisnis> {
  final newsRepo = Get.put(TempoNewsRepository());
  final String author = "Tempo Bisnis - Berita Bisnis Terkini Indonesia dan Dunia";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/tempo/bisnis';
  ...
  void processData(BeritaModel data) async {
    ...
    TempoNewsRepository.instance.setNullListJudulBisnisTempoNews();
    await Future.delayed(const Duration(milliseconds: 100));
    int tempCtr = TempoNewsRepository.instance.getCountPeriod();
    await TempoNewsRepository.instance.getAllNewsTempoBisnis(tempCtr);
    await TempoNewsRepository.instance.getAllNewsTempoBisnis(tempCtr-1);
    await TempoNewsRepository.instance.getAllNewsTempoBisnis(tempCtr-2);
    await TempoNewsRepository.instance.getAllNewsTempoBisnis(tempCtr-3);
    List<String> listJudul = TempoNewsRepository.instance.getListJudulBisnisTempoNews();
