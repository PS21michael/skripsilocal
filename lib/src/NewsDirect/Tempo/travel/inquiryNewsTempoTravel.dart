...
class InquiryTempoNewsTravel extends StatefulWidget {
  const InquiryTempoNewsTravel({Key? key}) : super(key: key);

  @override
  State<InquiryTempoNewsTravel> createState() => _InquiryTempoNewsTravelState();
}

class _InquiryTempoNewsTravelState extends State<InquiryTempoNewsTravel> {
  final newsRepo = Get.put(TempoNewsRepository());
  final String author = "Tempo Travel - Berita Terbaru Wisata Indonesia dan Dunia";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/tempo/travel';
  ...
  void processData(BeritaModel data) async {
    ...
    TempoNewsRepository.instance.setNullListJudulTravelTempoNews();
    await Future.delayed(const Duration(milliseconds: 100));
    int tempCtr = TempoNewsRepository.instance.getCountPeriod();
    await TempoNewsRepository.instance.getAllNewsTempoTravel(tempCtr);
    await TempoNewsRepository.instance.getAllNewsTempoTravel(tempCtr-1);
    await TempoNewsRepository.instance.getAllNewsTempoTravel(tempCtr-2);
    await TempoNewsRepository.instance.getAllNewsTempoTravel(tempCtr-3);
    List<String> listJudul = TempoNewsRepository.instance.getListJudulTravelTempoNews();
