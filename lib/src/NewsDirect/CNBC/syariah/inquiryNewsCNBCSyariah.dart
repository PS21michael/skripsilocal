...
class InquiryCNBCNewsSyariah extends StatefulWidget {
  const InquiryCNBCNewsSyariah({Key? key}) : super(key: key);

  @override
  State<InquiryCNBCNewsSyariah> createState() => _InquiryCNBCNewsSyariahState();
}

class _InquiryCNBCNewsSyariahState extends State<InquiryCNBCNewsSyariah> {
  final newsRepo = Get.put(CNBCNewsRepository());
  final String author = "Syariah - Berita Terkini Ekonomi Syariah - CNBC Indonesia";
  final String publisher = "CNBC Indonesia";
  final String category = "Syariah";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/cnbc/syariah';
  ...
  void processData(BeritaModel data) async {
    ...
    await CNBCNewsRepository.instance.getAllNewsCNBCSyariah(tempCtr-2);
    await CNBCNewsRepository.instance.getAllNewsCNBCSyariah(tempCtr-3);
    List<String> listJudul = CNBCNewsRepository.instance.getListJudulSyariahCNBCNews();
