...
class InquiryCNBCNewsLifeStyle extends StatefulWidget {
  const InquiryCNBCNewsLifeStyle({Key? key}) : super(key: key);

  @override
  State<InquiryCNBCNewsLifeStyle> createState() => _InquiryCNBCNewsLifeStyleState();
}

class _InquiryCNBCNewsLifeStyleState extends State<InquiryCNBCNewsLifeStyle> {
  final newsRepo = Get.put(CNBCNewsRepository());
  final String author = "Lifestyle - Berita Terkini Gaya Hidup dan Travel - CNBC Indonesia";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/cnbc/lifestyle';
  ...
  void processData(BeritaModel data) async {
    ...
    CNBCNewsRepository.instance.setNullListJudulLifeStyleCNBCNews();
    await Future.delayed(const Duration(milliseconds: 100));
    int tempCtr = CNBCNewsRepository.instance.getCountPeriod();
    await CNBCNewsRepository.instance.getAllNewsCNBCLifeStyle(tempCtr);
    await CNBCNewsRepository.instance.getAllNewsCNBCLifeStyle(tempCtr-1);
    await CNBCNewsRepository.instance.getAllNewsCNBCLifeStyle(tempCtr-2);
    await CNBCNewsRepository.instance.getAllNewsCNBCLifeStyle(tempCtr-3);
    List<String> listJudul = CNBCNewsRepository.instance.getListJudulLifeStyleCNBCNews();
