...
class InquiryMerdekaNewsTeknologi extends StatefulWidget {
  const InquiryMerdekaNewsTeknologi({Key? key}) : super(key: key);

  @override
  State<InquiryMerdekaNewsTeknologi> createState() => _InquiryMerdekaNewsTeknologiState();
}

class _InquiryMerdekaNewsTeknologiState extends State<InquiryMerdekaNewsTeknologi> {
  final newsRepo = Get.put(MerdekaNewsRepository());
  final String author = "Merdeka News | Teknologi";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/merdeka/teknologi';
  ...
  void processData(BeritaModel data) async {
    ...
    MerdekaNewsRepository.instance.setNullListJudulTeknologiMerdekaNews();
    await Future.delayed(const Duration(milliseconds: 100));
    int tempCtr = MerdekaNewsRepository.instance.getCountPeriod();
    await MerdekaNewsRepository.instance.getAllNewsMerdekaTeknologi(tempCtr);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaTeknologi(tempCtr-1);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaTeknologi(tempCtr-2);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaTeknologi(tempCtr-3);
    List<String> listJudul = MerdekaNewsRepository.instance.getListJudulTeknologiMerdekaNews();
