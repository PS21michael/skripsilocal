...
class InquiryMerdekaNewsOlahraga extends StatefulWidget {
  const InquiryMerdekaNewsOlahraga({Key? key}) : super(key: key);

  @override
  State<InquiryMerdekaNewsOlahraga> createState() => _InquiryMerdekaNewsOlahragaState();
}

class _InquiryMerdekaNewsOlahragaState extends State<InquiryMerdekaNewsOlahraga> {
  final newsRepo = Get.put(MerdekaNewsRepository());
  final String author = "Merdeka News | Olahraga";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/merdeka/olahraga';
  ...
  void processData(BeritaModel data) async {
    ...
    MerdekaNewsRepository.instance.setNullListJudulOlahragaMerdekaNews();
    await Future.delayed(const Duration(milliseconds: 100));
    int tempCtr = MerdekaNewsRepository.instance.getCountPeriod();
    await MerdekaNewsRepository.instance.getAllNewsMerdekaOlahraga(tempCtr);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaOlahraga(tempCtr-1);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaOlahraga(tempCtr-2);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaOlahraga(tempCtr-3);
    List<String> listJudul = MerdekaNewsRepository.instance.getListJudulOlahragaMerdekaNews();
