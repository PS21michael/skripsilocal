...
class InquiryMerdekaNewsJateng extends StatefulWidget {
  const InquiryMerdekaNewsJateng({Key? key}) : super(key: key);

  @override
  State<InquiryMerdekaNewsJateng> createState() => _InquiryMerdekaNewsJatengState();
}

class _InquiryMerdekaNewsJatengState extends State<InquiryMerdekaNewsJateng> {
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/merdeka/jateng';
  ...
  void processData(BeritaModel data) async {
    ...
    await MerdekaNewsRepository.instance.getAllNewsMerdekaJateng(tempCtr-2);
    await MerdekaNewsRepository.instance.getAllNewsMerdekaJateng(tempCtr-3);
    List<String> listJudul = MerdekaNewsRepository.instance.getListJudulJatengMerdekaNews();
