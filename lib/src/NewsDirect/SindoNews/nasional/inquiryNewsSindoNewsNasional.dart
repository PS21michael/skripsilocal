...
class InquirySindoNewsNasional extends StatefulWidget {
  const InquirySindoNewsNasional({Key? key}) : super(key: key);

  @override
  State<InquirySindoNewsNasional> createState() => _InquirySindoNewsNasionalState();
}

class _InquirySindoNewsNasionalState extends State<InquirySindoNewsNasional> {
  final newsRepo = Get.put(SindoNewsRepository());
  final String author = "Berita Nasional Terkini dan Terbaru- SINDOnews";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/sindonews/nasional';
  ...
  void processData(BeritaModel data) async {
    ...
    SindoNewsRepository.instance.setNullListJudulNasionalSindoNews();
    await Future.delayed(const Duration(milliseconds: 100));
    int tempCtr = SindoNewsRepository.instance.getCountPeriod();
    await SindoNewsRepository.instance.getAllNewsSindoNewsNasional(tempCtr);
    await SindoNewsRepository.instance.getAllNewsSindoNewsNasional(tempCtr-1);
    await SindoNewsRepository.instance.getAllNewsSindoNewsNasional(tempCtr-2);
    await SindoNewsRepository.instance.getAllNewsSindoNewsNasional(tempCtr-3);
    List<String> listJudul = SindoNewsRepository.instance.getListJudulNasionalSindoNews();
