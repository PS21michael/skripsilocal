...
class InquiryOkezoneNewsBola extends StatefulWidget {
  const InquiryOkezoneNewsBola({Key? key}) : super(key: key);

  @override
  State<InquiryOkezoneNewsBola> createState() => _InquiryOkezoneNewsBolaState();
}

class _InquiryOkezoneNewsBolaState extends State<InquiryOkezoneNewsBola> {
  final newsRepo = Get.put(OkezoneNewsRepository());
  final String author = "Sindikasi bola.okezone.com";
  ...
  Future<BeritaModel?> getData() async {
    try {
      String url = 'https://api-berita-indonesia.vercel.app/okezone/bola';
  ...
  void processData(BeritaModel data) async {
    ...
    OkezoneNewsRepository.instance.setNullListJudulBolaOkezoneNews();
    await Future.delayed(const Duration(milliseconds: 100));
    ...
    await OkezoneNewsRepository.instance.getAllNewsOkezoneBola(tempCtr-3);
    List<String> listJudul = OkezoneNewsRepository.instance.getListJudulBolaOkezoneNews();
